...
// With no memory accesses yet, initialize both the access and hit 
counters.
...
blocks = new int[number_of_lines_in_cache, 
number_of_words_in_block];
// Now figure out how many bits there are for each component of the 
address, i.e.,
// how many bits for the tag and how many bits for the line id.
number_of_line_bits = MAXIMUM_NUMBER_OF_LINE_BITS - 
number_of_word_bits;
number_of_tag_bits = NUMBER_OF_BITS_IN_ADDRESS - number_of_line_bits 
- number_of_word_bits;
// From the number of bits in each address component, we can now create 
the
...
DirectCache* cache;
cache = new 
DirectCache(number_of_bits_defining_block_size);
...
// cache.
for (int i = 0; i < 
powerOfTwo(NUMBER_OF_BITS_IN_ADDRESS); i++)
...
#endif
#ifdef SEQ2
int instruction_sequence[] = {int main(int argc, char* argv[])
{
// Declare the array of memory.
memory = new char[powerOfTwo(NUMBER_OF_BITS_IN_ADDRESS)];
// Ask user for the number of bits for the word id
...
cout << "The percentage of hits is: " << cache-
>getPercentageOfHits() << "\n";
}
else
{
cout << "Error: The number of bits entered must be 1, 2, 
3, 4, or 5.";
}
return 0;
}
